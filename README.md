...
The controller probes a node with SSH and nvidia-smi.

A free GPU is selected using utilization, memory, reservation, and lock state.

SLERB atomically creates one lock directory per allocated GPU.

A remote job directory is created under the node's SLERB scratch directory.

The input directory and run script are copied to the node with rsync.

The run script executes inside the staged work directory with GPU and array metadata exported as environment variables.

Logs, state, exit status, and staged files remain in the remote job directory until fetched or cleaned.
...
The downloadable file is identical to the README above.
